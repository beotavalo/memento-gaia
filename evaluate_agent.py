...
class OpenAIBackend(ChatBackend):
    ...
    async def chat(
        ...
        # Use max_completion_tokens for newer models (gpt-5, gpt-4.1*, gpt-4o*, o1/o3/o4),
        # fallback to max_tokens for legacy chat-completions models.
            ...
            try:
                ...
                for tool in tools_resp.tools:
                    if tool.name in self.sessions:
                        logger.warning(f"Duplicate tool name '{tool.name}' from {script}, skipping")
                        continue
                    self.sessions[tool.name] = session
                    connected_tools.append(tool.name)
                    
                logger.info(f"Successfully connected to {script}")
                
            except Exception as e:
                logger.error(f"Failed to connect to {script}: {e}")
                continue
                
        if not connected_tools:
            raise RuntimeError("No server tools could be connected. Please check server scripts and paths.")
            
        print(f"Connected tools ({len(connected_tools)}): {connected_tools}")
    ...
    async def process_query(self, query: str, file: str, task_id: str = "interactive") -> str:
        ...
        for cycle in range(self.MAX_CYCLES):
            ...
            for task in tasks:
                ...
                while True:
                    exec_msgs = trim_messages(exec_msgs, MAX_CTX, model=self.exec_llm.model)
...
def parse_args():
    ...
    parser.add_argument("--start", type=int, default=0, help="Start index of questions to evaluate.")
    parser.add_argument("--end", type=int, default=20, help="End index of questions to evaluate.")
    parser.add_argument("--task_id", type=str, default=None, help="Task ID of a specific question to evaluate.")
    parser.add_argument("-f", "--file", type=str, default="", help="Optional file path")
    parser.add_argument("-m", "--meta_model", type=str, default="gpt-5", help="Meta‑planner model")
    parser.add_argument("-e", "--exec_model", type=str, default="gpt-5", help="Executor model") # o3-2025-04-16
...
def load_gaia_data(data_path, start, end, task_id=None)->list:
    """Loads GAIA data from a jsonl file."""
    data = []
...
def calculate_score(agent_answer, correct_answer):
    """Calculates the score based on exact match."""
    return 1 if str(agent_answer).strip().lower() == str(correct_answer).strip().lower() else 0

async def test_gaia_reading(args):
    """Tests the agent on the GAIA reading dataset."""
    load_dotenv()
    args = parse_args()
    ...
    validation_data_path = os.path.join(os.path.dirname(__file__), 'dataset/GAIA/2023/validation/metadata.jsonl')
    questions = load_gaia_data(validation_data_path, args.start, args.end, args.task_id)
    ...
    for question in questions:
        file_path = ('dataset/GAIA/2023/validation/' + question['file_name']) if 'file_name' in question and question['file_name'] else ""
        print(f"Evaluating question: {question['task_id']}")
        print(f"The question: {question['Question']}")
        print (f"The question's file path: {file_path}")
        agent_answer = await run_single_query(client, question['Question'], file_path)
        score = calculate_score(agent_answer=agent_answer, correct_answer=question['Final answer'])
        result = {
            "task_id": question['task_id'],
            "question": question['Question'],
            "level": question['Level'],
            "agent_answer": agent_answer,
            "correct_answer": question['Final answer'],
            "score": score
        }
    ...
    filename = f"./gaia_run/evaluation_results_{timestamp}.jsonl"
    with open(filename, 'w') as f:
        for result in results:
            f.write(json.dumps(result) + '\n')

    accuracy = total_score / len(questions) if questions else 0
    print(f"\nEvaluation Summary:")
    print(f"Total questions: {len(questions)}")
    print(f"Correct answers: {total_score}")
    print(f"Accuracy: {accuracy:.2f}")
        
    return print("Tested GAIA reading dataset")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_gaia_reading(arg_ns))